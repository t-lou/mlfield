Here’s a clean, end‑to‑end PyTorch MTL template:  
- shared backbone  
- two heads (e.g., segmentation + depth or classification + aux)  
- uncertainty weighting  
- gradient logging hooks  
- easy to plug your own dataset/model in.

You can drop this into a single `mtl_template.py` and adapt.
...
- `TaskHeadA`, `TaskHeadB`: task‑specific heads  
- `MultiTaskModel`: wraps backbone + heads  
- `MTLLoss`: uncertainty‑weighted loss  
- `train_one_epoch`: core training loop with logging
...
class MTLLoss(nn.Module):
    ...
    """
    def __init__(self, loss_fn_a, loss_fn_b,
                 init_log_sigma_a: float = 0.0,
                 init_log_sigma_b: float = 0.0):
        super().__init__()
...
class DummyMTLDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        seg = torch.randint(0, self.num_classes,
...
def compute_grad_norms(model: nn.Module,
                       task_params: Dict[str, nn.Parameter]) -> Dict[str, float]:
    """
...
def main():
    ...
    optimizer = torch.optim.Adam(
        list(model.parameters()) + list(mtl_loss_fn.parameters()),
        lr=1e-3
    )
...
- Replace `SimpleBackbone` with your ResNet/ViT/DETR encoder.
- Replace `SegmentationHead` / `DepthHead` with:
  - detection head  
  - classification head  
  - keypoint head  
  etc.
- Replace `DummyMTLDataset` with a real dataset that returns:
  - `x: Tensor [B, C, H, W]`  
  - `targets["task_a"]`, `targets["task_b"]` in compatible shapes.

If you want, I can give you a variant of this template that’s:
- DETR‑style (multi‑decoder transformer), or  
- tailored for an autonomous‑driving dataset (e.g., segmentation + depth on Cityscapes/NYUv2).
